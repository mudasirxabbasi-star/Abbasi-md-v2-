...
async function stickercropCommand(sock, chatId, message) {
    ...
    try {
        ...
        if (finalSizeKB > 1000) {
            console.log(`Warning: Sticker size (${Math.round(finalSizeKB)} KB) exceeds recommended limit but will be sent anyway`);
        }
        ...
        const json = {
            ...
            'sticker-pack-name': settings.packname || 'ABBASI-XMD',
            'emojis': ['✂️']
        };
