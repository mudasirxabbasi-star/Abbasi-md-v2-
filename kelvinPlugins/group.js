...
},
{
    ...
    operate: async ({ kelvin, m, reply, isGroup, GroupDB, from, prefix }) => {
        if (!isGroup) return reply(global.mess.notgroup);
...
},
{
    ...
    operate: async ({ kelvin, text, m, reply, isGroup, from, prefix }) => {
...
},
{
    ...
    operate: async ({ kelvin, m, reply, isGroup,Access, participants, text, groupMetadata }) => {
        if (!isGroup) return reply(global.mess.notgroup);
...
{
        command: ['tagall'],
        operate: async ({ kelvin, m, reply, isGroup, participants, text }) => {
            if (!isGroup) return reply(global.mess.notgroup);
            if (!m.isAdmin) return reply(global.mess.notadmin);
            if (!m.isBotAdmin) return reply(global.mess.botadmin);
    ...
    {
        command: ['mute', 'close'],
        operate: async ({ kelvin, m, reply, isGroup, isBotAdmin }) => {
            if (!isGroup) return reply(global.mess.notgroup);
            if (!m.isAdmin) return reply(global.mess.notadmin);
            if (!m.isBotAdmin) return reply(global.mess.botadmin);
    ...
    {
        command: ['setdesc'],
        operate: async ({ kelvin, m, reply, isGroup, text }) => {
            if (!isGroup) return reply(global.mess.notgroup);
            if (!m.isAdmin) return reply(global.mess.notadmin);
            if (!m.isBotAdmin) return reply(global.mess.botadmin);
    ...
    {
        command: ['approve'],
        operate: async ({ kelvin, m, reply, isGroup, botNumber }) => {
            if (!isGroup) return reply(global.mess.notgroup);
            if (!m.isAdmin) return reply(global.mess.notadmin);
            if (!m.isBotAdmin) return reply(global.mess.botadmin);
    ...
    operate: async ({ kelvin, m, reply, isGroup, prefix, quoted, mime, args, command }) => {
...
},
{
    ...
    operate: async ({ kelvin, m, reply, args, prefix, Access, db, botNumber, mess }) => {
    ...
    },
    {
        ...
        operate: async ({ kelvin, m, reply, Access, isGroup, text, mentionedJid, quoted }) => {
            if (!isGroup) return reply(global.mess.notgroup);
            if (!isGroup) return reply(global.mess.notgroup);
            if (!m.isAdmin) return reply(global.mess.notadmin);
            if (!m.isBotAdmin) return reply(global.mess.botadmin);
    ...
    },
    {
        ...
        operate: async ({ kelvin, m, reply, isGroup, text, mentionedJid, quoted }) => {
            if (!isGroup) return reply(global.mess.notgroup);
            if (!m.isAdmin) return reply(global.mess.notadmin);
            if (!m.isBotAdmin) return reply(global.mess.botadmin);
    ...
    {
        command: ['editinfo'],
        operate: async ({ kelvin, m, reply, isGroup, args, prefix }) => {
            if (!isGroup) return reply(global.mess.notgroup);
            if (!m.isAdmin) return reply(global.mess.notadmin);
            if (!m.isBotAdmin) return reply(global.mess.botadmin);
    ...
    {
        command: ['closetime'],
        operate: async ({ kelvin, m, reply, isGroup, args }) => {
            if (!isGroup) return reply(global.mess.notgroup);
            if (!m.isAdmin) return reply(global.mess.notadmin);
            if (!m.isBotAdmin) return reply(global.mess.botadmin);
    ...
    {
        command: ['antilink'],
        operate: async ({ kelvin, m, reply, prefix, args, isGroup, db, botNumber }) => {
            if (!isGroup) return reply(global.mess.notgroup);
            if (!m.isAdmin) return reply(global.mess.notadmin);
            if (!m.isBotAdmin) return reply(global.mess.botadmin);
...
╰─────────❖`;
    ...
    {
        command: ['tagall2'],
        operate: async ({ kelvin, m, reply, isGroup, participants, from }) => {
            try {
                if (!isGroup) return reply(global.mess.notgroup);
                if (!m.isAdmin) return reply(global.mess.notadmin);
                if (!m.isBotAdmin) return reply(global.mess.botadmin);
    ...
    {
        command: ['add'],
        operate: async ({ kelvin, m, reply, prefix, isGroup, text, quoted }) => {
              if (!m.isGroup) return reply(global.mess.notgroup);
              if (!m.isAdmin) return reply(global.mess.notadmin);
            if (!m.isBotAdmin) return reply(global.mess.botadmin);
...
},
{
...
},
 {
    ...
    operate: async ({ kelvin, m, reply, isGroup, participants, quoted }) => {
        if (!isGroup) return reply(global.mess.notgroup);
            if (!m.isAdmin) return reply(global.mess.notadmin);
            if (!m.isBotAdmin) return reply(global.mess.botadmin);
