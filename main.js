...
// ABBASI-XMD owner number (replace with your actual number)
global.ownerNumber = "923071312885";

// ─────────────────────────────────────────────────────────────────────────────
// ABBASI-XMD NEWSLETTER CONTEXT (used in all forwarded messages)
// ─────────────────────────────────────────────────────────────────────────────
const channelInfo = {
    contextInfo: {
        forwardingScore: 1,
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        if (isBanned(senderId) && !userMessage.startsWith('.unban')) {
            ...
            if (Math.random() < 0.1) {
                await sock.sendMessage(chatId, {
                    text: '❌ You are banned from using ABBASI-XMD. Contact an admin to get unbanned.',
                    ...channelInfo
                });
            }
            return;
        ...
        if (isGroup && isAdminCommand) {
            ...
            if (!isBotAdmin) {
                await sock.sendMessage(chatId, { text: 'Please make ABBASI-XMD an admin to use admin commands.', ...channelInfo }, { quoted: message });
                return;
            }
        ...
        if (isOwnerCommand) {
            if (!message.key.fromMe && !senderIsOwnerOrSudo) {
                await sock.sendMessage(chatId, { text: '❌ This command is only available for ABBASI-XMD owner or sudo!' }, { quoted: message });
                return;
            }
        }
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.ban'):
                if (!isGroup) {
                    if (!message.key.fromMe && !senderIsSudo) {
                        await sock.sendMessage(chatId, { text: 'Only ABBASI-XMD owner/sudo can use .ban in private chat.' }, { quoted: message });
                        break;
                    }
                }
            ...
            case userMessage.startsWith('.unban'):
                if (!isGroup) {
                    if (!message.key.fromMe && !senderIsSudo) {
                        await sock.sendMessage(chatId, { text: 'Only ABBASI-XMD owner/sudo can use .unban in private chat.' }, { quoted: message });
                        break;
                    }
                }
            ...
            case userMessage.startsWith('.mode'):
                ...
                if (!message.key.fromMe && !senderIsOwnerOrSudo) {
                    await sock.sendMessage(chatId, { text: 'Only ABBASI-XMD owner can use this command!', ...channelInfo }, { quoted: message });
                    return;
                }
                ...
                if (!action) {
                    ...
                    await sock.sendMessage(chatId, {
                        text: `Current ABBASI-XMD mode: *${currentMode}*\n\nUsage: .mode public/private\n\nExample:\n.mode public - Allow everyone to use bot\n.mode private - Restrict to owner only`,
                        ...channelInfo
                    }, { quoted: message });
                    return;
                }
                ...
                try {
                    ...
                    await sock.sendMessage(chatId, { text: `ABBASI-XMD is now in *${action}* mode`, ...channelInfo });
                } catch (error) {
            ...
            case userMessage.startsWith('.anticall'):
                if (!message.key.fromMe && !senderIsOwnerOrSudo) {
                    await sock.sendMessage(chatId, { text: 'Only ABBASI-XMD owner/sudo can use anticall.' }, { quoted: message });
                    break;
                }
                {
            ...
            case userMessage.startsWith('.antilink'):
                ...
                if (!isBotAdmin) {
                    await sock.sendMessage(chatId, {
                        text: 'Please make ABBASI-XMD an admin first.',
                        ...channelInfo
                    }, { quoted: message });
                    return;
                }
            ...
            case userMessage.startsWith('.antibadword'):
                ...
                if (!isBotAdmin) {
                    await sock.sendMessage(chatId, { text: '*ABBASI-XMD must be admin to use this feature*', ...channelInfo }, { quoted: message });
                    return;
                }
            ...
            case userMessage.startsWith('.chatbot'):
                ...
                if (!chatbotAdminStatus.isSenderAdmin && !message.key.fromMe) {
                    await sock.sendMessage(chatId, { text: '*Only admins or ABBASI-XMD owner can use this command*', ...channelInfo }, { quoted: message });
                    return;
                }
            ...
            case userMessage.startsWith('.remini'):
                ...
                break;
            case userMessage.startsWith('.stickercrop') || userMessage.startsWith('.scrop'):
            ...
            case userMessage.startsWith('.pies '):
                ...
                break;
            case userMessage === '🥧' || userMessage === 'pie':
                await piesAlias(sock, chatId, message);
                break;
            case userMessage.startsWith('.anime'):
                await animeCommand(sock, chatId, message);
                break;
            case userMessage === '.heart' || userMessage === '❤️' || userMessage === '💕' || userMessage === '💗':
                await handleHeart(sock, chatId, message);
                break;
            case userMessage.startsWith('.misc') || userMessage.startsWith('.random'):
