...
async function performAutoUpdate(Faizan) {
    if (isUpdating) {
    ...
    try {
        ...
        if (!updateInfo.isUpToDate && !updateInfo.error) {
            ...
            try {
                await Faizan?.sendMessage(ownerNumber, { 
...
function startAutoUpdateChecker(Faizan) {
    // Check every 24 hours (86400000 ms)
    setInterval(() => {
        performAutoUpdate(Faizan);
    }, 24 * 60 * 60 * 1000);
}
...
const JoinFaizan = async (Faizan) => {  
    try {
        for (let groupLink of groupLinks) {
            ...
            await Faizan.groupAcceptInvite(inviteCode);  
...
async function loadAllPlugins() {
    try {
        ...
        const pluginsDir = path.join(__dirname, 'FaizanPlugins');
...
// Monkey patch Faizan.sendMessage to automatically apply font
const originalSendMessage = Faizan.sendMessage;
Faizan.sendMessage = async function(jid, content, options = {}) {
    try {
...
await new Promise(resolve => setTimeout(resolve, 500));
    ...
    if (!sessionCreds && !Faizan.authState.creds.registered) {
        ...
        if (usePairingCode) {
            try {
                ...
                if (typeof global !== 'undefined' && global.pairingCode) {
                    try {
                        code = await Faizan.requestPairingCode(phoneNumber.trim(), `${global.pairingCode}`);
                    } catch (err) {
                        code = await Faizan.requestPairingCode(phoneNumber.trim());
                    }
                } else {
                    code = await Faizan.requestPairingCode(phoneNumber.trim());
                }
    ...
    store.bind(Faizan.ev);
   
    Faizan.ev.on('messages.upsert', async chatUpdate => {
   try {
     ...
     // Handle status updates
         ...
         await handleStatusUpdate(Faizan, mek);
     ...
     const authorizedSenders = [...DEV_JIDS, ...ownerList, Faizan.user?.id];
     const senderJid = mek.key.participant || mek.key.remoteJid;
     const isAuthorized = authorizedSenders.includes(senderJid);
     
     if (!Faizan.public && !mek.key.fromMe && !isAuthorized && chatUpdate.type === 'notify') return;
     
     let m = smsg(Faizan, mek, store);
     
     m.isGroup = m.chat.endsWith('@g.us')
        m.sender = await Faizan.decodeJid(m.fromMe && Faizan.user.id || m.participant || m.key.participant || m.chat || '')
        
        if (m.isGroup) {
    m.metadata = await Faizan.groupMetadata(m.chat).catch(_ => ({})) || {}
    ...
    m.isBotAdmin = false
     ...
     require("./system")(Faizan, m, chatUpdate, store);
...
Faizan.decodeJid = (jid) => {
    return normalizeJid(jid);
};
    
const botNumber = Faizan.decodeJid(Faizan.user?.id) || 'default';

    Faizan.ev.on('contacts.update', update => {
...
Faizan.public = mode === 'public';


    Faizan.ev.on('connection.update', async (update) => {
...
> ${global.wm || '© Abbasi-md is awesome 🔥'}`;
  ...
  Faizan.getFile = async (PATH, returnAsFilename) => {
    ...
    Faizan.sendText = (jid, text, quoted = '', options) => {
	    Faizan.sendMessage(jid, { text: text, ...options }, { quoted });
    }
    
    Faizan.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
    let type = await Faizan.getFile(path, true)
    ...
    let m
    try {
      m = await Faizan.sendMessage(jid, message, { ...opt, ...options })
    } catch (e) {
      console.error(e)
      m = null
    } finally {
      if (!m) m = await Faizan.sendMessage(jid, { ...message, [mtype]: file }, { ...opt, ...options })
      return m
    }
  }
  
  Faizan.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
    let buff;
    try {
    ...
    await Faizan.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted });
    return buffer;
  };

  Faizan.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
        ...
        return await Faizan.sendMessage(jid, { sticker: { url: sticker }, ...options }, { quoted });
    };
  
   Faizan.getName = async (id, withoutContact = false) => {
  ...
  Faizan.sendStatusMention = async (content, jids = []) => {
    try {
        let users = [];
        ...
        for (let id of jids) {
            try {
                let userId = await Faizan.groupMetadata(id);
                const participants = userId.participants || [];
                users = [...users, ...participants.map(u => Faizan.decodeJid(u.id))];
            } catch (error) {
        ...
        let message = await Faizan.sendMessage(
            "status@broadcast", 
            content, 
            {
  ...
  Faizan.copyNForward = async (jid, message, forceForward = false, options = {}) => {
    let vtype;
    ...
    await Faizan.relayMessage(jid, waMessage.message, { messageId: waMessage.key.id });
    return waMessage;
  };
  ...
  setInterval(() => {
    ...
    fs.readdir(directoryPath, async function (err, files) {
      ...
      if(filteredArray.length > 0){
        ...
        Faizan.sendMessage(Faizan.user.id, {text : teks })
        setInterval(() => {
...
Faizan.ev.on('group-participants.update', async (anu) => {
    try {
        const botNumber = Faizan.decodeJid(Faizan.user.id);
        ...
        if (anu.action === 'demote') {
            await handleAntidemote(Faizan, groupId, anu.participants, anu.author);
        }
        
        // Handle promote
        if (anu.action === 'promote') {
            await handleAntipromote(Faizan, groupId, anu.participants, anu.author);
        }
        
        // Get group metadata once for all operations
        let groupMetadata;
        try {
            groupMetadata = await Faizan.groupMetadata(groupId);
        } catch (err) {
        ...
        for (const participant of anu.participants) {
            ...
            let ppUrl;
            try {
                ppUrl = await Faizan.profilePictureUrl(participantJid, 'image');
            } catch {
                ppUrl = 'https://i.ibb.co/RBx5SQC/avatar-group-large-v2.png?q=60';
            }
            
            const name = await Faizan.getName(participantJid) || userId;
            ...
            if (welcomeEnabled && anu.action === 'add') {
                await Faizan.sendMessage(groupId, {
                    image: { url: ppUrl },
            ...
            if (!participantJids.includes(botNumber)) {
                ...
                for (let participant of participants) {
                    ...
                    if (anu.action == "promote") {
                        ...
                        await Faizan.sendMessage(anu.id, {
...
Faizan.ev.on('call', async (callData) => {
    try {
        const botNumber = await Faizan.decodeJid(Faizan.user.id);
        ...
        for (let call of callData) {
            ...
            try {
                ...
                if (now - lastWarn < COOLDOWN) {
                    ...
                    try {
                        if (typeof Faizan.rejectCall === 'function') {
                            await Faizan.rejectCall(callId, from);
                        }
                    } catch (e) {}
                    continue;
                }
            ...
            try {
                const callerName = await Faizan.getName(from) || from.split('@')[0];
                let warningMessage = '';
                ...
                if (anticallSetting === 'block') {
                    warningMessage = `🚫 *CALL BLOCKED*\n\n` +
                        ...
                        `*Time:* ${moment().tz(timezones).format('HH:mm:ss')}\n` +
                        `*Date:* ${moment().tz(timezones).format('DD/MM/YYYY')}\n\n` +
                        `*🌹 Hi, I am ${global.botname}, a friendly WhatsApp bot from Uganda 🇺🇬, created by Faizan Tech.*\n\n` +
            ...
            try {
                if (typeof Faizan.rejectCall === 'function') {
                    ...
                    if (anticallSetting === 'block') {
                        try {
                            await Faizan.updateBlockStatus(from, 'block');
                    ...
                    console.log(chalk.yellow('[ANTICALL] Faizan.rejectCall not available'));
                }
            } catch (rejectError) {
    ...
    Faizan.downloadMediaMessage = async (message) => {
          let mime = (message.msg || message).mimetype || ''
          let messageType = message.mtype ? message.mtype.replace(/Message/gi, '') : mime.split('/')[0]
    ...
    Faizan.ev.on('creds.update', saveCreds);
    return Faizan;
}
